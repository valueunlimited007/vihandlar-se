...
// Copy of the specific risk mapping from complete-eadditives
function getSpecificENumberRisk(e_number: string, name: string): { risk_score: number; longevity_impact: string; origin: string; avoidance_tips: string[]; natural_alternatives: string[] } | null {
...
function getCategoryRiskData(category: string, name: string, e_number: string): { risk_score: number; longevity_impact: string; origin: string; avoidance_tips: string[]; natural_alternatives: string[] } {
  const lowerCategory = category.toLowerCase();
  const lowerName = name.toLowerCase();

  // Check for azo dyes
  if (lowerCategory.includes('färgämne') && (lowerName.includes('azo') || e_number.match(/E1[0-9][0-9]/))) {
    return {
      risk_score: 7,
      longevity_impact: 'Negativ',
      origin: 'Syntetiskt azofärgämne',
      avoidance_tips: ['Undvik särskilt för barn', 'Läs ingrediensförteckningar noga', 'Välj naturligt färgade produkter'],
      natural_alternatives: ['Naturliga färgämnen från frukt och grönsaker', 'Beta-karoten', 'Antocyaniner']
    };
  }

  // Check for nanoparticles
  if (lowerName.includes('nano') || lowerName.includes('titandioxid')) {
    return {
      risk_score: 7,
      longevity_impact: 'Negativ',
      origin: 'Syntetisk nanopartikel',
      avoidance_tips: ['Undvik särskilt för barn', 'Välj ekologiska alternativ', 'Läs ingrediensförteckningar noga'],
      natural_alternatives: ['Naturliga alternativ utan nanopartiklar', 'Kalciumkarbonat', 'Naturliga färgämnen']
    };
  }
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Fetching all published E-additives...');
    ...
    console.log(`Found ${existingAdditives.length} E-additives to update`);

    const updatedIds: string[] = [];
    let updatedCount = 0;

    // Process each E-additive
    for (const additive of existingAdditives) {
      try {
        console.log(`Processing ${additive.e_number} - ${additive.name}`);

        // Apply new risk scoring logic
        const specificRisk = getSpecificENumberRisk(additive.e_number, additive.name);
        const categoryRisk = getCategoryRiskData(additive.category, additive.name, additive.e_number);
        
        // Use specific risk if available, otherwise use category risk
        const riskData = specificRisk || categoryRisk;
        ...
        const updateData: EAdditiveUpdate = {
          ...
          avoidance_tips: riskData.avoidance_tips,
          natural_alternatives: riskData.natural_alternatives,
          meta_title: seoMeta.meta_title,
          meta_description: seoMeta.meta_description
        };

        // Update the E-additive
        ...
        if (updateError) {
          console.error(`Error updating ${additive.e_number}:`, updateError.message);
          continue;
        }
        ...
        console.log(`Successfully updated ${additive.e_number} with risk score ${riskData.risk_score}`);
    ...
    console.log(`Successfully updated ${updatedCount} E-additives`);

    return new Response(
      JSON.stringify({
        success: true,
        message: `Successfully updated ${updatedCount} E-additives with new risk scoring`,
        updatedCount,
        totalProcessed: existingAdditives.length,
        updatedIds: updatedIds.slice(0, 10) // Show first 10 IDs
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 200
    ...
    console.error('Error in update-existing-scores function:', error);
    return new Response(
      JSON.stringify({
        success: false,
        error: error.message
